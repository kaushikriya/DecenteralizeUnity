...
export const Transactions = ({
  ...
  const handleTransactionVote = async (transaction) => {
    await confirmTransaction(transaction.transactionId);
    setVoted(true);
  };

  const btnStyle =
    "justify-self-center rounded-full bg-[#f5f5f5] hover:bg-blue-200 cursor-pointer px-1 py-2 m-2 w-[20%] backdrop-filter backdrop-blur-sm bg-opacity-10 border border-gray-100";
              ...
              transactions.map((t) => (
                <div class="grid p-5 border border-gray-100 m-5 rounded-xl">
                  <div class="w-full flex justify-between">
                    <p class="w-[98%] font-bold antialiased text-lg">
                      {t.name}
                    </p>
